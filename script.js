...
const config = {
  ...
  calendarStart: "20260523T093000",
  calendarEnd: "20260523T113000",
  timezone: "Asia/Ho_Chi_Minh",

  // Photo
  photoSrc: "./assets/og.chuhieu03-mainpicture.png",
};
// ─────────────────────────────────────────────────────────────────────────────
...
function bindContent() {
  ...
  const bindings = {
    ...
    footerSubline: "From chuhieu03 with love ♥️",
  };
  ...
  ["left", "right"].forEach((side) => {
    for (let i = 0; i < 30; i++) {
      ...
      piece.style.setProperty(
        "--travel-x",
        side === "left" ? `${travelX}vw` : `-${travelX}vw`,
      );
...
function startMusic() {
  ...
  if (playPromise && typeof playPromise.catch === "function") {
    playPromise.catch(() => {
      // Autoplay can be blocked; user can enable via toggle.
    });
  }

  toggle.setAttribute("aria-pressed", "true");
  toggle.textContent = "Tat nhac";
}
